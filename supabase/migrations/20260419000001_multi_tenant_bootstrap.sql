-- ============================================
-- CEREBRO — Multi-tenant BOOTSTRAP (self-contained)
-- This migration creates tenants + all dependencies from scratch if they're missing.
-- Safe to re-run (all IF NOT EXISTS). Use instead of 20260419000000 if tenants table doesn't exist.
-- ============================================
...
-- If the table already existed without the stripe columns, add them safely
...
-- 1. PROFILES — ensure tenant_id + super_admin columns exist ----
-- The initial schema created `profiles` with tenant_id as UUID but without foreign key.
...
-- The initial schema created `integrations` with tenant_id as TEXT; we add OAuth columns.
...
-- 6. HELPER: accept invitation on signup + auto-create tenant for new signups
...
-- 9. BACKFILL: create a default tenant for any existing profile without one
DO $$
DECLARE
  r RECORD;
  v_tenant_id UUID;
...
-- 10. Make Kike's existing account a super_admin so he can access /admin
UPDATE public.profiles
SET is_super_admin = TRUE, role_system = 'super_admin'
WHERE lower(email) IN ('eduardoarnedog@gmail.com', 'eduardo@retorna.app', 'earnedo@retorna.app');
